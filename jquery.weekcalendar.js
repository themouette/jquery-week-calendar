  ...
  $.widget('ui.weekCalendar', (function() {
    ...
    return {
      ...
      prevWeek: function() {
        ...
        // daylight savings or other anomalies
  ...
  TODO    if total height for an hour is less than 11px, there is a display problem.
      ...
      _renderCalendarBodyFreeBusy: function($calendarTableTbody) {
        if (this.options.displayFreeBusys) {
          ...
          for (var i = 1; i <= options.daysToShow; i++) {
            // TODO: if options.displayFreeBusys is false, this loop is useless
            if (options.displayFreeBusys) {
              if (!showAsSeparatedUser) {
                renderRow += '<td class=\"wc-day-column day-' + i + '\">';
        ...
        $.each(sortedEvents, function() {
          ...
          if (curGroups.length > 0) {
            if (lastEndTime.getTime() <= $curEvent.data('calEvent').start.getTime()) {
              // finishes the current group list by adding it to the resulting
              // list of groups and cleans it
      ...
      _positionEvent: function($weekDay, $calEvent) {
        ...
        $calEvent.css({
          top: pxTop, height: pxHeight || (pxPerMillis * 3600000 / options.timeslotsPerHour
        )});
      },
       ...
       * the resized / dropped event to it's max possible duration  based on the
       * overlap. If no satisfactory adjustment can be made, the event is
        ...
        $calEvent.resizable({
          ...
          stop: function(event, ui) {
            ...
            if (self._needDSTdayShift($calEvent.data('calEvent').start, newEnd)) {
              wEnd = self._getDSTdayShift(newEnd, -1);
            }
